...
# The new variable x and its value should appear in the environment window.
...
apply(meta, 2, is.numeric)
str(meta)

# 3) What columns in meta are non-informative?
# First replace all strings indicating missing values with NA
meta[meta=='missing'] <- NA
# Then find the columns that are all NA
apply(meta, 2, function(x)all(is.na(x)))
# 4) Add the 9th value to the Fibonacci sequence stored in x
x <-- append(x, x[length(x)] + x[length(x)-1])
# 5) Make a boxplot of host fev1 as a function of host clinical state with 
# appropriate labels
pdf(file="out.pdf", height=4, width=4)
